...
import gen_mountain_mod as mountain
pygame.init()
class Window:
    run = True
    display_info = pygame.display.Info()
    width, height = display_info.current_w, display_info.current_h
    base_screen = pygame.display.set_mode((width, height), pygame.NOFRAME)
    offset = 24
    screen_rect = base_screen.get_rect().inflate(offset*2, offset*2)
    screen = pygame.Surface(screen_rect.size, pygame.SRCALPHA)
    ...
    m_lines = mountain.generate(width * 2, height, tilesize // 2)
    m_lines2 = mountain.generate(width * 2, height, tilesize // 2, 0.5)
    m_offset = -width // 2
    fonts = {}
    ...
    def draw_window(self):
        ...
        m1_offset = camera_x // 10 + self.m_offset
        m2_offset = camera_x // 5 + self.m_offset
        for line in self.m_lines:
            if line[0] + m1_offset <= 0 and line[0] + m1_offset > - (self.tilesize / 2) / 2:
                pygame.draw.line(self.screen, (128, 128, 144), (1, line[1]), (1, line[3]), width = self.tilesize // 2)
            pygame.draw.line(self.screen, (128, 128, 144), (line[0] + m1_offset, line[1]), (line[2] + m1_offset, line[3]), width = self.tilesize // 2)
        for line in self.m_lines2:
            pygame.draw.line(self.screen, (144, 144, 144), (line[0] + m2_offset, line[1]), (line[2] + m2_offset, line[3]), width = self.tilesize // 2)
        #camera_y = min(camera_x, max_offset[1])
        count = 0
        #drawing tiles
        ...
        self.base_screen.blit(self.screen, self.screen_rect)
    ...
    def keypress_inputs(self, key):
        if key == pygame.K_l:
            ...
            if self.is_submerged(PLAYER.rect) or self.is_grounded(PLAYER.rect, PLAYER):
